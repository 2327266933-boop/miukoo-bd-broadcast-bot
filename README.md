...
面向 BD 运营场景的网页工作台。使用者在页面里输入总户商家名称或 BD 名单，选择消息类型，预览后创建群发任务，并在同一个页面查看每个 BD 的回复状态、提醒状态和导出结果。

当前阶段先做网页工作台和本地演示发送；飞书机器人只是后续真实发送和接收回复的通道，不是使用者需要先配置的主体。
